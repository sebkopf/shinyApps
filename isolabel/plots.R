...
generate_plot1 <- reactive({
  ...
  p <- 
    data$plot1.df %>% 
    mutate(dblt = as.numeric(dblt), labeling_time = as.numeric(labeling_time)) %>% 
    ggplot() +
    ...
    scale_y_log10("labeling time",
                  expand = c(0, 0.1), breaks = data$plot1.df$labeling_time %>% as.numeric() %>% unique(),
                  labels = unique(data$plot1.df$labeling_time.label)) +
    scale_x_log10("generation time", expand = c(0, 0.2),
                  breaks = isolate(get_doubling_times()$dblt %>% as.numeric()),
